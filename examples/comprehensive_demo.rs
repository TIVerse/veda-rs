...
fn demo_parallel_iterators() {
    ...
    // Collect
    let doubled: Vec<i32> = vec![1, 2, 3, 4, 5].into_par_iter().map(|x| x * 2).collect();
    ...
    println!(
        "   Has value > 50: {}, All positive: {}",
        has_large, all_positive
    );
...
fn demo_priority_tasks() {
    ...
    println!(
        ...
        normal_priority_count.load(Ordering::Relaxed)
    );
...
fn demo_telemetry() {
    ...
    veda_rs::runtime::with_current_runtime(|rt| {
        ...
        println!(
            "   P99 latency: {:.2}μs",
            snapshot.p99_latency_ns as f64 / 1000.0
        );
...
fn demo_async_bridge() {
    ...
    let doubled = veda_rs::block_on(items.par_map(|x| async move { x * 2 }).collect());
